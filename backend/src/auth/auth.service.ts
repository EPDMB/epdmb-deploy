...
export class AuthService {
  ...
  async registerUser(user: RegisterUserDto): Promise<string> {
    ...
    const token = this.jwtService.sign({ email: newUser.email },{ secret: process.env.JWT_SECRET },);
  ...
  async registerSeller(sellerData: RegisterSellerDto,): Promise< string > {
    ...
    const token = this.jwtService.sign({ email: sellerData.email },{ secret: process.env.JWT_SECRET },);
  ...
  async sendEmailVerification(email: string, token: string) {
    ...
    await this.mailService.sendMail({to: email, subject: 'Confirma tu cuenta', template: 'mail', context: { url },});
  }
  ...
  async loginUser({ email, password }: LoginUserDto) {
      ...
      if (user.status === false)
        throw new UnauthorizedException('Verifica tu cuenta para poder iniciar sesión');
  ...
  async sendPasswordResetLink(email: string): Promise<void> {
    ...
    const token = this.jwtService.sign({ userId: user.id },{ secret: process.env.JWT_SECRET, expiresIn: '1h' },);
  ...
  async sendEmailResetPassword(email: string, token: string) {
    ...
    try {
      await this.mailService.sendMail({to: email, subject: 'Solicitud de restablecimiento de contraseña', template: 'passwordmail', context: { url },});
    } catch (error) {
      throw new InternalServerErrorException('Error al enviar el correo de restablecimiento de contraseña',);
    }
  }

  async resetPassword( token: string,
    resetPasswordDto: ResetPasswordDto, res: Response
  ): Promise<void> {
    try {
